...
public class MultiChannelSingle {
    static float delta = 1e-6f;

    private void MultiChannelTest(int l, int channels) {
        Random random = new Random();
        FourierTransform.Single forward = new FourierTransform.Single(l, false);

        float[] input = new float[l * channels * 2];
        float[] output = new float[l * channels * 2];
        float[] test = new float[l * channels * 2];
        float[] channelInput = new float[l * 2];
        float[] channelOutput = new float[l * 2];
        int i, j, k;
